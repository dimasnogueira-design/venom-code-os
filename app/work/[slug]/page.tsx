...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const project = getWorkCase(slug); return project ? { title: `${project.title} | Work | VENOM CODE`, description: project.description, alternates: { canonical: `/work/${project.slug}` } } : {}; }

export default async function WorkCasePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const project = getWorkCase(slug); if (!project) notFound();
