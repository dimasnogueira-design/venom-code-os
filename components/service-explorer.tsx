...
const services = [
  { id:"web", icon:Globe2, number:"01", title:"Websites", line:"A primeira impressão.\nUma presença inteira.", intro:"Da página de campanha a um site com conteúdo, atendimento e integrações. O formato acompanha o objetivo do negócio.", interest:"Site ou landing page", intent:"Quero uma presença digital mais forte", visual:["Sua marca.","Seu próximo nível."], offers:[
  ...
  { id:"ai", icon:Bot, number:"02", title:"IA & Automação", line:"Menos repetição.\nMais inteligência.", intro:"Conecte as etapas que hoje dependem de copiar, colar e responder tudo manualmente. IA onde ela ajuda, regras claras onde elas bastam.", interest:"Automação e IA", intent:"Quero automatizar tarefas e atendimento", visual:["Entrada","Entendimento","Ação"], offers:[
  ...
  { id:"commerce", icon:ShoppingCart, number:"03", title:"E-commerce", line:"Da descoberta\nao pedido.", intro:"Uma jornada de compra que combina apresentação, confiança e operação. A plataforma é escolhida depois de entender o catálogo e o modelo de venda.", interest:"E-commerce", intent:"Quero vender produtos online", visual:["Catálogo","Carrinho","Checkout"], offers:[
  ...
  { id:"systems", icon:Braces, number:"04", title:"Sistemas", line:"Sua operação.\nDo seu jeito.", intro:"Ferramentas construídas em torno do trabalho real: quem usa, o que precisa fazer e quais informações precisa acessar.", interest:"Sistema sob medida", intent:"Quero organizar minha operação", visual:["Pessoas","Processos","Visibilidade"], offers:[
  ...
  { id:"brand", icon:Palette, number:"05", title:"Branding", line:"Uma marca\nque ocupa espaço.", intro:"Estratégia e expressão visual trabalhando juntas. A identidade precisa fazer sentido na apresentação, no produto e no contato com o cliente.", interest:"Branding e identidade", intent:"Quero construir ou evoluir minha marca", visual:["Identidade","Expressão","Presença"], offers:[
  ...
  { id:"growth", icon:TrendingUp, number:"06", title:"Performance", line:"Medir. Aprender.\nEvoluir.", intro:"Uma presença digital precisa continuar melhorando depois da publicação. Primeiro, enxergar os gargalos; depois, priorizar o que mudar.", interest:"Performance e crescimento", intent:"Quero melhorar um site que já existe", visual:["Observar","Priorizar","Testar"], offers:[
  ...
  return <>
    <section className="experience-services section-pad" id="servicos">
      <div className="shell">
        ...
        <div className="experience-grid">{services.map(({icon:Icon,...item},i)=><button key={item.id} className={`experience-card experience-${item.id}`} onClick={()=>open(i)} aria-haspopup="dialog" aria-label={`Explorar ${item.title}`}>
          <span className="experience-card-top"><span>{item.number} / {item.title}</span><Icon size={22} aria-hidden="true" /></span>
          <span className={`service-visual visual-${item.id}`} aria-hidden="true">{item.visual.map((word,n)=><span key={word}><i>{String(n+1).padStart(2,"0")}</i>{word}</span>)}</span>
    ...
    <section className="solution-routing shell section-pad" aria-labelledby="solution-title"><div><p className="section-kicker">02 / SEU PRÓXIMO MOVIMENTO</p><h2 id="solution-title">Comece pelo problema.<br /><em>A solução vem depois.</em></h2><p>Você não precisa chegar com a tecnologia escolhida. Encontre a situação mais próxima da sua.</p></div><div className="solution-list">{services.map((s,i)=><button key={s.id} onClick={()=>open(i)} aria-haspopup="dialog"><span>{s.intent}</span><ArrowRight size={18}/></button>)}</div></section>
    <dialog ref={dialog} className="service-dialog" aria-labelledby="service-dialog-title" onClick={e=>{if(e.target===e.currentTarget)dialog.current?.close();}}>
      <div className="service-dialog-body">
        ...
        <p className="section-kicker">VENOM CODE / {service.number}</p><h2 id="service-dialog-title">{service.title}</h2><p className="service-dialog-intro">{service.intro}</p>
