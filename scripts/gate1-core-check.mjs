...
const failures = [
  ...
  ...["satisfies ServiceRecord[]", "satisfies CaseRecord"].filter((entry) => !records.includes(entry)).map((entry) => "missing typed record " + entry),
];
...
if (failures.length) {
  ...
  console.log("PASS Venom Core tokens and typed content records");
}
